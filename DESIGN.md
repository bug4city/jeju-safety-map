...
- `CctvLayer`: optional map overlay using compact white camera-icon markers with `--cctv` as the data accent. At close zoom each marker adds the camera count beside the icon so CCTV coverage is visible without competing with incident pins. Purpose labels clarify that the layer mixes traffic, facility, disaster, and safety cameras.
